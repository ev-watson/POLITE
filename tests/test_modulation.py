"""Modulation reductions: round-trip + flat-field independence."""
...
def test_noiseless_roundtrip_all_methods(q, u):
    ...
    a_ratio = pt.double_ratio(bfs4)
    a_diff = pt.double_difference(bfs4)
    b = pt.lsq_modulation(bfs8)
...
def test_double_ratio_flat_field_independent():
    """double_ratio is invariant to a per-beam throughput imbalance;
    ...
    ratio_m = pt.double_ratio(matched)
    ratio_i = pt.double_ratio(imbal)
    diff_i = pt.double_difference(imbal)
...
def test_double_ratio_requires_four_angles():
