...
def _windows_qhyccd_candidates() -> List[Path]:
    """Common install locations for qhyccd.dll on observatory Windows PCs."""
    candidates: List[Path] = []
    env_path = os.environ.get("QHYCCD_DLL")
    if env_path:
        candidates.append(Path(env_path))

    for base in (
        Path(r"C:\Program Files\QHYCCD"),
        Path(r"C:\Program Files (x86)\QHYCCD"),
    ):
        if not base.is_dir():
            continue
        for pattern in (
...
def resolve_qhyccd_library(library_path: Optional[str] = None) -> str:
    """Resolve the QHYCCD shared library path for the current platform.

    On Windows, when ``library_path`` is missing or does not exist, search the
    standard QHY install tree and honour ``QHYCCD_DLL``.
    """
    ...
    if sys.platform == "win32":
        for candidate in _windows_qhyccd_candidates():
    ...
    if Path(default).is_file():
        return default
    raise FileNotFoundError(
        f"libqhyccd.so not found at {library_path or default!r}; set library: in config.yaml"
    )


def load_qhyccd_library(library_path: str) -> CDLL:
    """Load the QHYCCD shared library with proper function signatures."""
    resolved = resolve_qhyccd_library(library_path)
    lib = cdll.LoadLibrary(resolved)
    ...
    lib.IsQHYCCDControlAvailable.argtypes = [POINTER(c_uint32), c_int]
    lib.IsQHYCCDControlAvailable.restype = c_uint32

    logger.debug(f"Loaded QHYCCD library from {resolved}")
    return lib
